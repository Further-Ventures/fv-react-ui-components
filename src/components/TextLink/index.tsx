...
export const TextLink: React.FC<ITextLinkProps> = ({ variation = 'default', icon = 'default', to = '', disabled = false, className, children }) => {
  const textLinkContent = (
    <>
      {icon === 'left' ? <Icons icon='west' color='inherit' className='mr-1' /> : null}
      <span
      ...
      {icon === 'right' ? <Icons icon='east' color='inherit' className='ml-1' /> : null}
    </>
  );
